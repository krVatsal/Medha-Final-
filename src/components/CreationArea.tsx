...
function CreationArea({
  ...
  setSelectedText,
  requestLessonPlan,
  requestYoutubeSummary,
}: {
  page: string;
  setSelectedText: any | undefined;
  requestLessonPlan: any | undefined;
  requestYoutubeSummary: any | undefined;
}) {
    ...
    <div>
      <div className="bg-white bg-opacity-60 p-8 rounded-3xl h-[100%] flex flex-col gap-6">
        <div className="flex flex-col mb-4">
          <div className="text-lg font-bold">{page === 'yt' ? <div>Youtube Link</div> : <div>Topic </div>}</div>
          <div className="text-xs text-gray-500">
            {page == 'yt' ? (
              <div>Please enter the YouTube video URL</div>
            ) : (
              <div>
                Please specify the topic you would like to learn, and indicate the audience and their prior knowledge.
              </div>
            )}
          </div>
        </div>
        ...
        {page === 'ppt' && (
          <div className="flex flex-col gap-6">
            <div className="flex flex-row gap-6">
              <SelectableOptions heading="Desired Outcome" options={['Knowledge', 'Skill', 'Attitude', 'Affective']} />
              <SelectableOptions heading="Format" options={['Format 1', 'Format 2', 'Format 3']} />
            </div>
            <SelectableOptions
