...
function ChatHistoryArea({ questions }: { questions: string[] }) {
  ...
  return (
    <div className="bg-white opacity-60 bg-opacity-60 p-6 rounded-3xl flex flex-col h-[410px] ">
          ...
          {questions.map((question, index) => (
            <button key={index} className="flex items-center mb-2 w-full text-left">
              <Image
                ...
                height={20}
              />
              <p className="text-sm flex-grow truncate">{truncateText(question, 30)}</p>
            </button>
          ))}
        </div>
      </div>
