...
export default function ExamForm() {
  ...
  const [isDataLoading, setIsDataLoading] = useState<boolean>(true); // Track data loading
  ...
  const fetchClassData = async () => {
    setIsDataLoading(true); // Set data loading state to true
    try {
      ...
      setIsDataLoading(false); // Set data loading state to false
    }
  };
  ...
  return (
    <div className="bg-white bg-opacity-60 p-6 rounded-2xl min-h-[410px] flex flex-col pt-4 flex-grow overflow-scroll ">
      ...
      </div>// Show skeletons while data is loading
      ) : (
